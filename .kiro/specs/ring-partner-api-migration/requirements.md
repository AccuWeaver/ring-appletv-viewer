...
- **Background_Refresh_Manager**: The component that pre-fetches device lists and snapshots during tvOS background app refresh cycles
...
- **Device_Authorization_Grant**: The OAuth 2.0 flow defined in RFC 8628, designed for input-constrained devices (smart TVs, consoles), where the device displays a code and the user authorizes on a separate device
...
6. The partner rate limit is 100 TPS as documented. The client enforces a token-bucket rate limiter below this threshold.
...
11. WHEN the authorization server returns a `slow_down` error during polling, THE Auth_Service SHALL increase the polling interval by 5 seconds before the next poll attempt
12. WHEN the device code expires before the user completes authorization, THE Auth_Service SHALL display a message prompting the user to restart the linking process
...
2. WHEN the Partner API returns a JSON:API response, THE Device_Service SHALL parse the `data` array, extracting each device's `id`, `type`, and `attributes` (including device name, model, firmware version, power source, and online status)
...
7. THE Device_Service SHALL map the Partner API `model` attribute to the domain `DeviceType` enum using a best-effort match, falling back to `.unknown` for unrecognized model strings
8. WHEN the Partner API device resource includes a `status` attribute, THE Device_Service SHALL map it to the `isOnline` property of the domain model; WHEN no status attribute is present, THE Device_Service SHALL default `isOnline` to `true`
...
9. IF the HTTP DELETE to the session resource URL fails, THEN THE Stream_Session_Manager SHALL log the error and still close the local `RTCPeerConnection` (best-effort cleanup)

### Requirement 4: Media Download (Video Clips and Snapshots)

**User Story:** As a tvOS app user, I want to play back recorded video clips and view recent snapshots from my cameras, so that I can review footage and quickly check the current view without starting a live stream.
...
3. IF the Partner API returns an HTTP 404 for a video download (event without recorded video), THEN THE Media_Service SHALL map the error to `PartnerAPIError.notFound` and propagate it to the caller
4. WHEN the user requests a snapshot for a device, THE Media_Service SHALL send a POST request to `https://api.amazonvision.com/v1/devices/{device_id}/media/image/download` with the Bearer access token
5. WHEN the Partner API returns image data, THE Media_Service SHALL provide the raw image data (JPEG or PNG) to the caller
6. IF the Partner API returns an HTTP error for any media request, THEN THE Media_Service SHALL map the error to a descriptive `PartnerAPIError` and propagate it to the caller
...
5. THE Event_Service SHALL sort events in descending order by `createdAt` and enforce a client-side maximum of 50 events, consistent with the existing behavior
6. WHEN the Partner API supports a `limit` query parameter, THE Event_Service SHALL pass `limit=50` in the request to reduce unnecessary data transfer

### Requirement 6: Webhook Event Notifications (Deferred)
...
### Requirement 7: Partner API Error Handling and Rate Limiting
...
4. THE Partner_API_Client SHALL enforce a client-side request rate below 100 transactions per second using a token-bucket rate limiter to stay within the partner rate limit
...
6. WHEN the `Retry-After` header is absent on a 429 response, THE Partner_API_Client SHALL use a default backoff of 1 second, doubling on each subsequent retry

### Requirement 8: Legacy Code Removal and Cleanup

**User Story:** As a developer, I want to remove all private API and SIP signaling code, so that the codebase only contains supported, maintainable API integrations.
...
3. THE Partner_API_Client SHALL operate without any SIP signaling code — the `SIPSignalingClient` class, `SIPError` enum, and all SIP-related models (`sipServerIp`, `sipServerPort`, `sipSessionId`, `sipFrom`, `sipTo`, `sipToken`, `sipEndpoints`) SHALL be deleted from the codebase
4. THE `DevicesWrapper` struct (which parses the private API's grouped device response format) SHALL be deleted and replaced with a JSON:API parser
5. THE `RingDeviceResponse` model SHALL be replaced with a Partner API-specific DTO that maps JSON:API `data` resources to the domain `RingDevice` model
6. THE `RingEventResponse` model SHALL be replaced with a Partner API-specific DTO that maps the Partner API event format to the domain `RingEvent` model
7. THE `StreamSessionResponse` model (which captures SIP session parameters) SHALL be deleted, as WHEP sessions do not use SIP parameters
8. THE `RingAPIError` enum SHALL be replaced by the `PartnerAPIError` enum, removing private-API-specific cases (`twoFactorRequired`, `twoFactorInvalid`, `noSnapshotAvailable`) and adding Partner API-specific cases
9. THE `DefaultWebRTCStreamService` SHALL be replaced by the `StreamSessionManager`, which uses the WHEP_Client for SDP exchange instead of SIP signaling
10. THE `WebRTCStreamService` protocol SHALL be replaced by `StreamSessionManagerProtocol`, accepting a device ID and power source type as input (instead of a `StreamSessionResponse` with SIP parameters)
11. THE `Constants.API` enum SHALL be updated to reference Partner API base URLs (`https://api.amazonvision.com/v1`) and remove all private API endpoint constants (`clients_api`, `doorbots`)
12. THE `AppConfiguration.maxStreamDuration` property SHALL be removed or deprecated, as stream duration is now determined by the device's `PowerSource` (30s battery / 60s line-powered) rather than a global configuration value

### Requirement 9: Domain Model Adaptation
...
5. THE `AuthToken` model SHALL include an optional `clientId` scope property if provided by the Partner API token response, while retaining `accessToken`, `refreshToken`, and `expiresAt`
6. THE `RingDevice` model SHALL retain the `DeviceType` enum with existing cases, mapping from the Partner API `model` attribute with a fallback to `.unknown` for unrecognized values

### Requirement 10: Background Refresh Migration

**User Story:** As a tvOS app user, I want the app to continue pre-fetching snapshots in the background, so that camera thumbnails are ready when I open the app.

#### Acceptance Criteria

1. THE Background_Refresh_Manager SHALL use the updated `Device_Service` (which returns `[RingDevice]` with `String` IDs) for fetching the device list during background refresh
2. THE Background_Refresh_Manager SHALL use the `Media_Service` for snapshot fetching (replacing the deprecated `SnapshotService`)
3. THE Background_Refresh_Manager SHALL pass `String` device IDs to the `Media_Service.downloadSnapshot(deviceId:)` method
4. WHEN a snapshot download fails for an individual device during background refresh, THE Background_Refresh_Manager SHALL silently skip that device and continue with remaining devices
5. THE Background_Refresh_Manager SHALL pre-fetch snapshots for up to 10 devices per background refresh cycle, consistent with existing behavior
